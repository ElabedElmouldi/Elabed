...
# --- 1. الإعدادات الأساسية (تأكد من وضع بياناتك هنا) ---
TOKEN = "8439548325:AAHOBBHy7EwcX3J5neIaf6iJuSjyGJCuZ68"
FRIENDS_IDS = ["5067771509", "2107567005"]
DATA_FILE = "bot_data.json"
APP_URL = "https://your-app-name.onrender.com" # رابط السيرفر الخاص بك
...
# ثوابت الاستراتيجية (إدارة 1000$ مقسمة على 10 صفقات)
MAX_TRADES = 10
TRADE_AMOUNT_USD = 100.0
SCAN_INTERVAL = 300        # مسح كل 5 دقائق
STOP_LOSS_PCT = 0.02       # وقف خسارة 2%
ACTIVATION_PROFIT = 0.04   # بدء تتبع الربح عند +4%
TRAILING_GAP = 0.02        # فارق المطاردة 2%
...
# --- 5. المراقبة اللحظية وتقارير الساعة ---
def monitor_thread():
            ...
            for s in list(active_trades.keys()):
                ...
                if cp > trade.get('highest_price', 0):
                    ...
                    if not trade.get('tr_act', False) and ((cp - trade['entry']) / trade['entry']) >= ACTIVATION_PROFIT:
                        ...
                        send_telegram(f"🚀 *تفعيل التتبع:* `{s}`")
                        save_data()
...
# --- 6. المحرك الرئيسي (القناص) ---
def main_engine():
    send_telegram("🛡️ *Mouldi Sniper v220.0 Active*")
    ...
    while True:
        try:
            ...
            if now >= last_scan + timedelta(seconds=SCAN_INTERVAL):
                if not is_btc_safe():
                    send_telegram("⚠️ *تنبيه:* BTC هابط، تم تخطي المسح.")
                ...
                if res_sorted:
                    # تقرير المسح
                    report = "🔍 *نتائج المسح (أفضل 5):*\n"
                    for i, r in enumerate(res_sorted[:5]):
                        report += f"{i+1}. `{r['s']}` | سكور: `{r['d'][0]}`\n"
                    send_telegram(report)

                    # الدخول التلقائي في المركز الأول
                    best = res_sorted[0]
                    if best['d'][0] >= 12 and best['s'] not in active_trades and
