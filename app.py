...
# --- 5. المراقبة اللحظية (تعديل الأخطاء) ---
def monitor_thread():
            ...
            for s in list(active_trades.keys()):
                ...
                if cp > trade.get('highest_price', 0):
                    ...
                    if not trade.get('tr_act', False) and ((cp - trade['entry']) / trade['entry']) >= ACTIVATION_PROFIT:
                        ...
                        send_telegram(f"🚀 تفعيل التتبع: {s} (+4%)")
                        save_data()
...
# --- 7. المحرك الرئيسي ---
def main_engine():
    send_telegram("🛡️ Sniper v210.0 Online")
    ...
    while True:
        try:
            ...
            if now >= last_scan + timedelta(seconds=SCAN_INTERVAL):
                if not is_btc_safe():
                    send_telegram("⚠️ BTC يهبط، تم تأجيل المسح.")
                ...
                if res_sorted:
                    ...
                    if best['d'][0] >= 12 and best['s'] not in active_trades and len(active_trades) < MAX_TRADES:
                        ...
                        send_telegram(f"🔔 دخول: {best['s']}\n💰 سعر: {best['d'][1]}")
                        save_data()
                else:
                    send_telegram("📡 تم المسح: لا توجد فرص.")
...
app = Flask('')
@app.route('/')
def home(): return "Mouldi Sniper v210.0 is Running"

if __name__ == "__main__":
    # تشغيل الخيوط بشكل منظم
